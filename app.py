# app.py
"""
Flask backend for verifier without generate_key HTML route.
- Discord OAuth (identify) with secure state signing
- Session-based login with cross-site cookies so static IONOS site can query /portal/me
- /portal/me to report login status (used by your JS to toggle buttons)
- Frontend redirects back to IONOS pages (index/games/privacy/donate/admin)
- Optional owner-only override helpers and role checks
- No /generate_key and no /use_key endpoints
...
def save_state():
    try:
        with open(STATE_FILE, "w") as f:
            json.dump({"global_override": global_override,
                       "admin_overrides": admin_overrides}, f)
    except Exception:
...
# -----------------------------------------------------------------------------
# OAuth endpoints (no "copy ID" page; no /generate_key route)
# -----------------------------------------------------------------------------
...
def _discord_callback():
    ...
    # Redirect straight back to the IONOS homepage (static)
    return redirect(IONOS_INDEX)
