...
# Admin /me used by the frontend to check admin status
...
# Admin generate custom key (admin only)
...
# Admin create override (GLOBAL or discord id) - duration in seconds
...
def admin_api_create_override():
    ...
    if id_ != "GLOBAL" and (not str(id_).isdigit() or not (16 <= len(str(id_)) <= 21)):
        return jsonify({"ok": False, "message": "invalid discord id"}), 400
    expires_at = int(time.time()) + duration
    try:
        if id_ == "GLOBAL":
            set_global_override(True)
            # store a timestamped override entry in admin_overrides state if you want
...
def admin_api_list_overrides():
    if not is_owner_session():
        return jsonify({"ok": False, "message": "admin only"}), 403
    try:
        # combine global_override flag and admin_overrides dict
        overrides = []
        if global_override:
...
def admin_api_validate_key():
    ...
    if not key:
        return jsonify({"ok": False, "message": "no key provided"}), 400
    try:
        # Reuse existing validation logic (use get_key_record and overrides/global_override)
        now = int(time.time())
        if global_override:
            return jsonify({"ok": True, "reason": "global override active"}), 200
        # check admin_overrides for any mapped DID
        # Note: you might want to check with a DID parameter in the future
        with sqlite3.connect(DB_PATH) as conn:
        ...
        if expires_at and expires_at < now:
            return jsonify({"ok": False, "message": "expired", "expires_at": expires_at}), 200
        return jsonify({"ok": True, "key": r[0], "did": r[1], "expires_at": expires_at}), 200
    except Exception:
